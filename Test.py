...
class Application(Frame):
	...
	def createWidgets(self):
		...
		self.close = Button(text="close", command=self.quit)
		self.close.pack()
		self.close.place(relx=0.6, rely=0.8)
		
		#button that opens a filename prompt
		self.openfile = Button(text="select a file", command=self.get_file)
		self.openfile.pack()
		self.openfile.place(relx=0.6, rely=0.3)
	
		self.runcode = Button(text="start analyzing!", command=self.run_code)
		self.runcode.pack()
